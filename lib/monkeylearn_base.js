...
MonkeylearnBase.prototype.internal_callback = function(deferred, classifier_response, https_response) {
    ...
    https_response.on('end', function() {
        ...
        if (error) {
            ...
            deferred.resolve(classifier_response) // fulfills the promise with `classifier_response` as the value
        }
    });
}
...
MonkeylearnBase.prototype.startRequest = function(method, path, payload, callback) {
    ...
    var req = https.request(
        request_options,
        this.internal_callback.bind(this, deferred, classifier_response)
    );
